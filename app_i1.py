...
def main():
    ...
    start_button = st.sidebar.button("Luna - Race Engineer")
    ...
    if start_button:
        st.sidebar.write("Say 'goodbye' to end the session.")
        run_voice_chat()
...
def run_voice_chat():
    ...
    timestamp = time.strftime("%Y.%m.%d-%Hhr_%Mmin_%Ssec")
    ...
    with open(log_filename, 'w') as log_file:
        ...
        st.sidebar.text(f"Subprocess completed. Logs are saved to: {log_filename}")
